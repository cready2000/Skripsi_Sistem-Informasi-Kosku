                            ...
                            <div class="card mb-3" style="margin:0 auto;max-width:50rem;">
                                <div class="card-body">
                                    <?php foreach ($datakamar as $dk) : ?>
                                        ...
                                        <form action="<?= base_url('pemesanan') ?>" method="post" name="form" onsubmit="return confirm_gender();">
                                            <center>
                                                <input type="hidden" value="<?= $dk['ID_KAMAR']; ?>" name="id_kamar">
                                                ...
                                                <input type="hidden" value="<?= $dk['GENDER_KOS']; ?>" id="gender" name="gender">
...
<script type="text/javascript">
    let gender = "<?php echo session()->get('jenis_kelamin_penyewa'); ?>";

    function confirm_gender() {
        // console.log(gender);
        // console.log('gender hidden = ' + $('#gender').val());
        if ($('#gender').val() == gender || $('#gender').val() == "Campur") {
            return true;
        } else {
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'Jenis kelamin Anda tidak sesuai',
                footer: '<span style="color:red">*Silahkan pilih kos sesuai dengan jenis kelamin Anda</span>'
            })
            return false;
        }
    }
